...
import { PUERHUB_CHAT_AUTH_HEADER } from '@/const/auth';
import {
...
export const POST = async (req: Request) => {
  let agentRuntime: LobeGoogleOpenAI;

  // ============  1. init chat model   ============ //

  try {
    // get Authorization from header
    const authorization = req.headers.get(PUERHUB_CHAT_AUTH_HEADER);
    ...
    agentRuntime = await LobeGoogleOpenAI.fromAPIKey(apiKey);
  } catch (e) {
