...
import { PUERHUB_CHAT_AUTH_HEADER, OAUTH_AUTHORIZED } from '@/const/auth';
...
beforeEach(() => {
  request = new Request(new URL('https://test.com'), {
    headers: {
      [PUERHUB_CHAT_AUTH_HEADER]: 'Bearer some-valid-token',
...
describe('POST handler', () => {
  describe('init chat model', () => {
    ...
    it('should return Unauthorized error when PUERHUB_CHAT_AUTH_HEADER is missing', async () => {
      const mockParams = { provider: 'test-provider' };
    ...
    it('should have pass clerk Auth when enable clerk', async () => {
      ...
      const request = new Request(new URL('https://test.com'), {
        ...
        body: JSON.stringify({ model: 'test-model' }),
        headers: {
          [PUERHUB_CHAT_AUTH_HEADER]: 'some-valid-token',
          [OAUTH_AUTHORIZED]: '1',
        },
      });
  ...
  describe('chat', () => {
    it('should correctly handle chat completion with valid payload', async () => {
      ...
      request = new Request(new URL('https://test.com'), {
        headers: { [PUERHUB_CHAT_AUTH_HEADER]: 'Bearer some-valid-token' },
        method: 'POST',
